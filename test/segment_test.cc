...
bool NaiveImplementation(
  chess_engine::Segment segment,
  chess_engine::Coordinates point
) {
  ...
  stop.rank += step.rank;
  for (
    chess_engine::Coordinates current = segment.start;
    current != stop;
    current.file += step.file, current.rank += step.rank
  ) {
    if (current == point) {
      return true;
    }
...
chess_engine::Segment RandomSegment(std::mt19937* generator) {
  ...
  chess_engine::Coordinates delta =
    {random_delta(*generator), random_delta(*generator)};
